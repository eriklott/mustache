...
func Example_partial() {
	t := mustache.NewTemplate()
	t.Parse("main", `Hello {{>mypartial}}`)
	t.Parse("mypartial", `World!`)
	t.Render(os.Stdout, "main", nil)
	// "Hello World!"
}
...
func Example_lambdaSection() {
	...
	t := mustache.NewTemplate()
	t.Parse("main", `{{#lambda}}World!{{/lambda}}`)
	t.Render(os.Stdout, "main", map[string]interface{}{"lambda": lambda})
	// "Hello World!"
}
...
func Example_dataStruct() {
	...
	t := mustache.NewTemplate()
	t.Parse("main", `My name is {{Name}}, and I am {{age}} years old.`)
	t.Render(os.Stdout, "main", data)
